...
public class ProviderService : IProviderService
{
    ...
    //TODO refactor these configurations to use IOptions<T> and pull from appSettings
    private readonly TimeSpan _minimumReservationDateTimeAllowed =  TimeSpan.FromDays(1);
    private const int _minimumTimeBlockInMinutes = 15;
    ...
    public bool CreateTimeSlots(int providerId, DateTimeOffset startTime, DateTimeOffset endTime)
    {
        ...
        if (startTime.Minute % _minimumTimeBlockInMinutes != 0 || endTime.Minute % _minimumTimeBlockInMinutes != 0) return false;
    ...
    private IEnumerable<TimeSlot> GenerateTimeSlots(int providerId, DateTimeOffset truncatedStartTime, DateTimeOffset truncatedEndTime)
    {
        ...
        {
            slotEndTime = slotStartTime.AddMinutes(_minimumTimeBlockInMinutes);
    ...
    public bool ReserveTimeSlot(Guid timeSlotId, int clientId)
    {
        ...
        if (timeSlot.StarTime < DateTimeOffset.Now + _minimumReservationDateTimeAllowed)
        {
            return false;
        }
